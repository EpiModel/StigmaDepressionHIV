...
fr1 <- paste0(tblnam, min(unique(fulldata$scenario.num)), sep = "")
fr2 <- paste0(tblnam, max(unique(fulldata$scenario.num)), sep = "")

saveRDS(fulldata, paste0(save_dir, "/fulldata_mddtbl2", fr1, "_", fr2,".rds"))




#B. Get outcome_sims  data  ------------------------------------------------------------
# output_dir <- paste0("C:/Users/Uonwubi/OneDrive - Emory University/Desktop/Personal/RSPH EPI Docs/RA2/GitRepos/StigmaDepressionHIV_real/data/intermediate/processed_tbl2_05092024")
# fulldata<-readRDS(paste(output_dir,'/fulldata_mddtbl2B.rds', sep=""))
...
saveRDS(outcomessims, paste0(save_dir, "/outcomessims_mddtbl2", fr1, "_", fr2, ".rds"))
...
saveRDS(outcomessce, paste0(save_dir, "/outcomessce_mddtbl2", fr1, "_", fr2, ".rds"))
...
#d. Remove files -----------------------------------------------------------------------
#remove sim files except for 001 reduced files
redusims_dir <- paste0("data/intermediate/scenarios_mddtbl2/redusims")

all_files <- dir(path = sims_dir, pattern = "^df__.*rds$")
files_to_keep <- dir(path = sims_dir, pattern = "^df__.*001.*rds$")
