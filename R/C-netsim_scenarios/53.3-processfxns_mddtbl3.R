...
process_fulldata <- function(file_name, ts) {
  ...
  d <- d %>%
    ...
    arrange(batch_number, sim) %>%
    mutate(sim2 = ifelse(batch_number > 1,
                         sim + ((cumsum(!duplicated(batch_number)) - 1) * 32),
                         sim)) %>%
    select(-sim) %>%
    rename(sim=sim2) %>%
    mutate(
...
get_niapiannt <- function(d) {
  ...
  d %>%
    ...
    select(
      ...
      mdd.txcurr.numall, mdd.mhcare.numrcvanytx, mdd.mhcare.numrcvminadeqtx, mdd.txstart.numall
    ) %>%
    group_by(tbl, scenario.num, scenario.new, scenario_name, sim) %>%
    summarise(across(c(incid, incid.mdd0, incid.mdd1,
                       mdd.txcurr.numall, mdd.mhcare.numrcvanytx,
                       mdd.mhcare.numrcvminadeqtx, mdd.txstart.numall),
                     ~ sum(.x, na.rm = TRUE)))  %>%
    ...
    mutate(nnt_txcurr = mdd.txcurr.numall / nia,
           ...
           nnt_minadeqtx = mdd.mhcare.numrcvminadeqtx / nia,
           nnt_txstart = mdd.txstart.numall / nia) %>%
    ungroup() %>%
    select(tbl, scenario.num, scenario.new, scenario_name, sim,
           starts_with("nia"), starts_with("pia"), starts_with("nnt"))
}
